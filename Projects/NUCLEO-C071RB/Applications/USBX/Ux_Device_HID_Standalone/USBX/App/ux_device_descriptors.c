...
#pragma data_alignment=4
#endif /* defined ( __ICCARM__ ) */
__ALIGN_BEGIN uint8_t USBD_HID_UPS_ReportDesc[]
__ALIGN_END =
{
  /* USER CODE BEGIN USBD_HID_UPS_ReportDesc                   */
  ...
  /* ============================================================
   * FEATURE REPORTS - For Windows HID Battery driver queries
   * Windows reads these via GET_REPORT control requests
   * ============================================================ */

  /* Static battery configuration flags - FEATURE */
  ...
  /* Static battery capacity fields - FEATURE */
  ...
  /* Dynamic battery fields - FEATURE with Volatile flag */
  ...
  0xB1, 0x82,        /*     Feature (Data,Var,Abs,Vol)         */
  0x75, 0x0C,        /*     Report Size (12) - padding         */
  0x95, 0x01,        /*     Report Count (1)                   */
  0xB1, 0x01,        /*     Feature (Const) - padding          */

  /* ============================================================
   * INPUT REPORTS - For asynchronous interrupt endpoint updates
   * macOS and async Windows updates use these
   * ============================================================ */

  /* Static battery configuration flags - INPUT */
  0x09, 0x8B,        /*     Usage (Rechargeable)               */
  0x09, 0x2C,        /*     Usage (Capacity Mode)              */
  0x15, 0x00,        /*     Logical Minimum (0)                */
  0x25, 0x01,        /*     Logical Maximum (1)                */
  ...
  /* Static battery capacity fields - INPUT */
  0x09, 0x83,        /*     Usage (DesignCapacity)             */
  0x09, 0x67,        /*     Usage (FullChargeCapacity)         */
  0x15, 0x00,        /*     Logical Minimum (0)                */
  0x27, 0xFF, 0xFF, 0x00, 0x00, /* Logical Maximum (65535)   */
  ...
  /* Dynamic battery fields - INPUT with Volatile flag */
  0x09, 0x66,        /*     Usage (RemainingCapacity)          */
  0x15, 0x00,        /*     Logical Minimum (0)                */
  0x27, 0xFF, 0xFF, 0x00, 0x00, /* Logical Maximum (65535)   */
  0x75, 0x10,        /*     Report Size (16)                   */
    ...
    case CLASS_TYPE_HID:
      ...
      switch(pdev->tclasslist[pdev->classId].InterfaceType)
      {
        ...
        case INTERFACE_HID_UPS:
          ...
          {
            /* Assign IN Endpoint */
            USBD_FrameWork_AssignEp(pdev, USBD_HID_UPS_EPIN_ADDR,
                                    USBD_EP_TYPE_INTR, USBD_HID_UPS_EPIN_HS_MPS);
          }
          else
          {
            /* Assign IN Endpoint */
            USBD_FrameWork_AssignEp(pdev, USBD_HID_UPS_EPIN_ADDR,
                                    USBD_EP_TYPE_INTR, USBD_HID_UPS_EPIN_FS_MPS);
          }
                                     ...
                                     uint32_t pConf, uint32_t *Sze)
{
  ...
  switch(pdev->tclasslist[pdev->classId].InterfaceType)
  {
    ...
    case INTERFACE_HID_UPS:
      ...
      pHidDesc->bNumDescriptors = 0x01U;
      pHidDesc->bHIDDescriptorType = 0x22U;
      pHidDesc->wDescriptorLength = USBD_HID_ReportDesc_length(INTERFACE_HID_UPS);
      *Sze += (uint32_t)sizeof(USBD_HIDDescTypedef);
      ...
      if (pdev->Speed == USBD_HIGH_SPEED)
      {
        ...
        __USBD_FRAMEWORK_SET_EP(pdev->tclasslist[pdev->classId].Eps[0].add,
                                USBD_EP_TYPE_INTR,
                                (uint16_t)pdev->tclasslist[pdev->classId].Eps[0].size,
                                USBD_HID_UPS_EPIN_HS_BINTERVAL,
                                USBD_HID_UPS_EPIN_FS_BINTERVAL);
      }
      else
      {
        /* Append Endpoint descriptor to Configuration descriptor */
